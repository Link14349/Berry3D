...
namespace Berry3D {
    ...
    public:
        Cube(float w = 1, float h = 1, float d = 1) : Entity(sqrt(w * w + h * h + d * d)) {
#define BERRY3D_CUBE_POINTS_DEF(W, H, D, ID) points.push_back(new Vector3(W w, H h, D d));
#define BERRY3D_PLANES_DEF(NX, NY, NZ, ID0, ID1, ID2) planes.push_back(new PlaneEle(new Vector3(NX, NY, NZ), ID0, ID1, ID2));
